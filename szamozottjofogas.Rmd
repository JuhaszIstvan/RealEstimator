---
title: "Jófogás ingatlan scraping process"
output:
  html_document:
    df_print: paged
editor_options:
...
hirdetescrawler<-function(adurl,adAr=0){
  ...
  #add Price 
  #price is a bit tricky to mine from this site because it has 2different values with 2 hidden.
...
CleanUpLakásData<-function(hirdetéstábla){
  ...
  #update: If the  date is larger(later) t han the query time, the script decreases it  by one year. it will produce odd numbers for ads who had been active for more than a year.
...
MapAds<-function(CheckUrl,SessionID,ProjectName)
{
  ...
  #Define the threshold in respect of return number in order to be able to prevent/interrupt executions that might spam the website.
  ...
  }else if(totáltalálat_kesz>0)
    {
      ...
      repeat
        ...
        if(!exists("masszlista")){
          masszlista<-hirdeteslista(webpage)
        }else{
          masszlista<-rbind(masszlista,hirdeteslista(webpage))    
        }
        ...
        cat(paste('CL: ',CheckUrl,"w8ing ", format(round(maszkido,3),nsmall=3), " masodpercet\n"))
        Sys.sleep(maszkido)
        #next repeat loop!
        }
      ...
      if(nrow(masszlista)>0)
        {
        #drop duplicate items (reranked entries during execution)
        masszlista<-masszlista[!duplicated(masszlista[,2]),]
        masszlista$SessionID<-SessionID
        masszlista$ProjectName<-ProjectName
        
        #save the data
        #write.csv(masszlista, paste(SessionID,"_SUMM.csv",sep=''), row.names=FALSE)

        return(masszlista)
        #save data
        }
      else
      {cat("ERROR: the return table isemptyyyyyyy")
...
if (!file.exists(TableOfProjectsFile)){
  print("ERROR: Project name is not unambigous, breaking!")
} else {
  print("A file létezik!")
  TableOfProjects<-readRDS(file=TableOfProjectsFile)
}

if (!exists("TableOfProjects")){
  print("ERROR: No Project Master File available!")
} else {
...
#if we have an URL available, we should scratch it into our tracker, and begin interpreting it.
if(!exists("CheckUrl")){
...
#The script urrently uses the sorted filelist of the project directory. this is not robust an requies adherence to a naming pattern. A proper solution is using the SessionTable to track the files as it was originally intended to be done. 
  ProjectCurrentAdListFile<-paste(SessionID,"_SUMM.Rda",sep='')
  saveRDS(ProjectCurrentAdList, file=ProjectCurrentAdListFile)

  ProjectAdList<-readRDS(file=ProjectAdListFile, refhook =NULL)
  file.copy(ProjectAdListFile, paste(ProjectAdListFile,"_",format(Sys.time(),"%Y%m%d_%H%M"),"_backup.Rda",sep=''))
...
```{r Feldolgozas}
#merging the data with the basicdata table. The definitions should be moved to the top. 
#Load the cumulated dataset

#ProjectCurrentAdListPattern<-"^JofogasLakasokBPFull\\_[0-9]{8}\\_[0-9]{4}[0-9A-Za-z]{2}\\_SUMM\\.Rda$"
#ProjectCurrentAdListFile<-sort(list.files(pattern = ProjectCurrentAdListPattern),decreasing=TRUE)[1]
...
rm(ProjectCurrentAdDetailList)

#check the ads that have disappeared  
#I am using the SQL query instead of normal data.frame operations because this kind of data is exactly the kind of data that is best stored in a proper RDBMS. Unfortunately, sqlite can't handle ouoter joins. 
...
AdsCurrentThatHaveDisappearedFile<-paste(SessionID,"_AdsClosed.Rda",sep='')
...
if(nrow(AdsThatHaveDisappeared)>0){
  ...
  if(CurrentSessionRecord$SessionID==SessionID)
  {
    ...
    file.copy(ProjectAdDetailListFile, paste(ProjectAdDetailListFile,"_",format(Sys.time(),"%Y%m%d_%H%M"),"_backup.Rda",sep=''))
...
#Checking if there are ads that need to be reopened. The entries which are collected, BUT have end datetime in the main Table. 
...
AdsThatHaveBeenReopenedFile<-paste(SessionID,"_AdsOpened.Rda",sep='')
...
#producing the list of new ads compared to the previous list
rm(AdsToBeCollected)
...
saveRDS(SessionTable, file="SessionTable.Rda")
ProjectCurrentAdDetailListFile<-paste("Detailedhirdetes_",format(Sys.time(),"%Y_%m_%d_%H%M%S"),".Rda",sep='')

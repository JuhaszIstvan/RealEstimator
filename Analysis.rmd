...
```{r dropping_columns}
AnalysisAdDetailList<-AnalysisAdDetailList[,!(names(AnalysisAdDetailList) %in% c("Helyseg","Meret","Ar","Deviance","Szobakszama", "Kategoria","PublishedTime","QueryTime","QueryState", "Havikozoskoltseg"))]
```
...
#test population
DedupeTestSet<-AnalysisAdDetailList[AnalysisAdDetailList$SzobaSzamCleared=="3" & AnalysisAdDetailList$Kerulet=="XVIII." & AnalysisAdDetailList$MeretCleared==74,]

#larger DedupeTestSet
DedupeTestSet<-AnalysisAdDetailList[AnalysisAdDetailList$Kerulet=="XVIII.",]

  CompareBy<-c("SiteID","Description","MeretCleared","Kerulet","EmeletOrg","ArMil","PublishedTimeCleared")
...
#calculating the number comparison pairs
...
#adding fuzzy comparison, the numbers seem to be working out well.
...
fuzzyrpairsDefault<-compare.dedup(DedupeTestLimited, blockfld = Blockfields, strcmp = which(colnames(DedupeTestLimited) %in% c("Description")), exclude=which(colnames(DedupeTestLimited) %in% ExcludeTheseFields))
...
#the the stochastic method ( sans underlying stochastic modell, use manual fixing)
...
testSignified<-sign(testMatrix>0)
testSignified[!upper.tri(testSignified)] <- 0
...
Adlinks$Description.3<-ifelse(Adlinks$PublishedTimeCleared.1<=Adlinks$PublishedTimeCleared.2,Adlinks$Description.1, Adlinks$Description.2)
Adlinks$Description.4<-ifelse(Adlinks$PublishedTimeCleared.1<=Adlinks$PublishedTimeCleared.2,Adlinks$Description.2, Adlinks$Description.1)

Adlinks$SiteID.1<-Adlinks$SiteID.3
Adlinks$SiteID.2<-Adlinks$SiteID.4
Adlinks$Description.1<-Adlinks$Description.3
Adlinks$Description.2<-Adlinks$Description.4
...
Adlinks$Description.3<-NULL
Adlinks$Description.4<-NULL

...
DedupeTestSet<-AnalysisAdDetailList[AnalysisAdDetailList$Kerulet=="XVIII.",]
  ...
  CompareBy<-c("SiteID","Description","MeretCleared","Kerulet","EmeletOrg","ArMil")
  Blockfields<-c("Kerulet","MeretCleared","EmeletOrg")
  ExcludeTheseFields<-c("SiteID")
...
fuzzyrpairsDefault<-compare.dedup(DedupeTestLimited, blockfld = Blockfields, strcmp = which(colnames(DedupeTestLimited) %in% c("Description")), exclude=ExcludeTheseFields)
...
for(index in 1:nrow(AdPairs)){
  replace(Adpairs$SiteID.1, Adpairs$SiteID.1==Adpairs[index,"SiteID.2"], Adpairs[index,"SiteID.1"])
